...
public class Middleware implements IApi {
    ...
    public String getMsgFromSocket(Socket socket) {
        ...
        try {
            ...
            data = in.readLine();
            // System.out.println("\r\nMessage from " +
            // socket.getInetAddress().getHostAddress() + ": " + data);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return data;
    ...
    public void discoverGroupManager() {
        ...
        try {
            ...
            while (i != 4) {
                ...
                packet = new DatagramPacket(msg, msg.length, InetAddress.getByName(MultiCastGroupAddress),
                        MultiCastPort);
            ...
            // System.out.println("We will communicate with Group Manager at Address +" +
            // CommunicationChannel.getInetAddress().getHostAddress() + "Port:"+
            ...
            while (true) {
                ...
                if (!newGroup2.getType().equals("noMessage")) {
                    ...
                    if (!middlewareTeamsBuffer.containsKey(teams)) {
                        // Se periptwsh pou erthei neo view dhmiourgoume tous antistoixous buffers pou
                        // xreiazomaste
                        gSock = teams;
                        GroupMessages group = new GroupMessages();
                        // ArrayList<Integer> fifo = new ArrayList<>();//gia ton kathena prosthetei
                        // lista me ta ids twn mhnymatwn
                        ...
                        for (int counter = 0; counter < newGroup2.getView().getMembers().size(); counter++) {
                            ...
                            ArrayList<Integer> list = new ArrayList<>();// gia ton kathena prosthetei lista me ta ids
                                                                        // twn mhnymatwn
                            mids.put(newGroup2.getView().getMembers().get(counter).getMemberPort(), list);
                            ArrayList<Integer> list2 = new ArrayList<>();// gia ton kathena prosthetei lista me ta ids
                                                                         // twn mhnymatwn

                            rids.put(newGroup2.getView().getMembers().get(counter).getMemberPort(), list2);
                            ArrayList<Integer> list3 = new ArrayList<>();// gia ton kathena prosthetei lista me ta ids
                                                                         // twn mhnymatwn

                            appids.put(newGroup2.getView().getMembers().get(counter).getMemberPort(), list3);
                            ArrayList<Integer> list4 = new ArrayList<>();// gia ton kathena prosthetei lista me ta ids
                                                                         // twn mhnymatwn

                            mbufids.put(newGroup2.getView().getMembers().get(counter).getMemberPort(), list4);
                            ArrayList<Integer> list5 = new ArrayList<>();// gia ton kathena prosthetei lista me ta ids
                                                                         // twn mhnymatwn
                            totalbufids.put(newGroup2.getView().getMembers().get(counter).getMemberPort(), list5);

                            ArrayList<Integer> list6 = new ArrayList<>();// gia ton kathena prosthetei lista me ta ids
                                                                         // twn mhnymatwn
                        ...
                        for (int counter = 0; counter < newGroup2.getView().getMembers().size(); counter++) {
                            if (gro.getFifoOrders()
                                    .containsKey(newGroup2.getView().getMembers().get(counter).getMemberPort())) {
                                continue;
                            } else {
                                // Buffers poy kratane ta id twn paketwn pou exoume parei
                                ArrayList<Integer> list = new ArrayList<>();// gia ton kathena prosthetei lista me ta
                                                                            // ids twn mhnymatwn
                                mids.put(newGroup2.getView().getMembers().get(counter).getMemberPort(), list);
                                ArrayList<Integer> list2 = new ArrayList<>();// gia ton kathena prosthetei lista me ta
                                                                             // ids twn mhnymatwn

                                rids.put(newGroup2.getView().getMembers().get(counter).getMemberPort(), list2);
                                ArrayList<Integer> list3 = new ArrayList<>();// gia ton kathena prosthetei lista me ta
                                                                             // ids twn mhnymatwn

                                appids.put(newGroup2.getView().getMembers().get(counter).getMemberPort(), list3);
                                ArrayList<Integer> list4 = new ArrayList<>();// gia ton kathena prosthetei lista me ta
                                                                             // ids twn mhnymatwn

                                mbufids.put(newGroup2.getView().getMembers().get(counter).getMemberPort(), list4);
                                ArrayList<Integer> list5 = new ArrayList<>();// gia ton kathena prosthetei lista me ta
                                                                             // ids twn mhnymatwn
                                totalbufids.put(newGroup2.getView().getMembers().get(counter).getMemberPort(), list5);

                                gro.getFifoOrders().put(newGroup2.getView().getMembers().get(counter).getMemberPort(),
                                        0);

                                ArrayList<Integer> list6 = new ArrayList<>();// gia ton kathena prosthetei lista me ta
                                                                             // ids twn mhnymatwn
                                idsCoord.put(newGroup2.getView().getMembers().get(counter).getMemberPort(), list6);
                            }
                        }
                    ...
                    afterchangeview(newGroup2.getView()); // Afou erthei allagh sto view koitame mhpws kapoios efyge apo
                                                          // ta paketa pou exoume hdh parei gia na ton svhsoume
                    //
                    int getout = -1;
                    while (getout != 0) {
                        getout = 0;
                ...
                if (middlewareTeamsBuffer.size() > 0) {
                    try {
                        ...
                        DatagramPacket packet = new DatagramPacket(newbuffer, newbuffer.length,
                                InetAddress.getLocalHost(), OurPort);
                        ...
                        int l = -1;
                        for (int i = 0; i < middlewareTeamsBuffer.get(receiveMessage.getGroupId()).getMembers()
                                .size(); i++) {
                            if (middlewareTeamsBuffer.get(receiveMessage.getGroupId()).getMembers().get(i)
                                    .getMemberPort() == receiveMessage.getSenderPort()) {
                                l++;
                            }
                        }
                        if (l < 0) {
                        ...
                        if (receiveMessage.getMembersSend().size() > middlewareTeamsBuffer
                                .get(receiveMessage.getGroupId()).getMembers().size()) {
                            int k = -1;
                            int size = 0;
                            // System.out.println("PERAF"+ receiveMessage.getMembersSend().size());
                            for (int i = 0; i < receiveMessage.getMembersSend().size(); i++) {
                                for (int j = 0; j < middlewareTeamsBuffer.get(receiveMessage.getGroupId()).getMembers()
                                        .size(); j++) {
                                    if (receiveMessage.getMembersSend().get(i) != middlewareTeamsBuffer
                                            .get(receiveMessage.getGroupId()).getMembers().get(j).getMemberPort()) {
                                        size++;
                                    }
                                }

                                if (size == middlewareTeamsBuffer.get(receiveMessage.getGroupId()).getMembers()
                                        .size()) {
                                    k = i;
                                    break;
                                }
                        ...
                        if (receiveMessage.getTotal() == 1) {
                            if (!rids.get(receiveMessage.getStartingSender()).contains(receiveMessage.getSeqNo())) {
                                if (!idsCoord.get(receiveMessage.getStartingSender())
                                        .contains(receiveMessage.getSeqNo())) {
                                    if (middlewareTeamsBuffer.get(receiveMessage.getGroupId()).getCoInfo().getCoMember()
                                            .getMemberPort() == OurPort) {
                                        int deliver = middlewareTeamsBuffer.get(receiveMessage.getGroupId()).getCoInfo()
                                                .getDeliverno();
                                        deliver++;
                                        // System.out.println("EIMAI O COORDINATOR ///BAZW TTON EPOEMNO SEIRIAKO ARITHMO
                                        // DELIVER :"+ deliver);
                                        middlewareTeamsBuffer.get(receiveMessage.getGroupId()).getCoInfo()
                                                .setDeliverno(deliver);
                                        receiveMessage.setTotaldelivered(deliver);
                                        for (int i = 0; i < middlewareTeamsBuffer.get(receiveMessage.getGroupId())
                                                .getMembers().size(); i++) {
                                            int doesExist = -1;

                                            for (int j = 0; j < receiveMessage.getMembersSend().size(); j++) {
                                                if (middlewareTeamsBuffer.get(receiveMessage.getGroupId()).getMembers()
                                                        .get(i)
                                                        .getMemberPort() == receiveMessage.getMembersSend().get(j)) {
                                                    doesExist = 1;
                                                }
                                            }
                                            if (doesExist == -1) {
                                                receiveMessage.getMembersSend()
                                                        .add(middlewareTeamsBuffer.get(receiveMessage.getGroupId())
                                                                .getMembers().get(i).MemberPort);
                                            }
                                        }
                        ...
                        if (whatIsendTotal.size() > 0) {
                            ...
                            if (whatIsendTotal.get(0).getMembersSend().size() > lastview.getMembers().size()) {
                                ...
                                if (k >= 0) {
                                    for (int j = 0; j < lastview.getMembers().size(); j++) {
                                        for (int i = 0; i < sendsBuffer.get(k).getMembersSend().size(); i++) {
                                            if (lastview.getMembers().get(j).getMemberPort() == sendsBuffer.get(k)
                                                    .getMembersSend().get(i)) {
                                                resend = 1;
                                            }
                                        }
                                    }
                                ...
                                BM_send(whatIsendTotal.get(globalchoicewhatItotal),
                                        whatIsendTotal.get(globalchoicewhatItotal).getGroupId());
                            ...
                            if (whatIsend.get(0).getMembersSend().size() > lastview.getMembers().size()) {
                                ...
                                BM_send(whatIsend.get(globalchoiceWhatI),
                                        whatIsend.get(globalchoiceWhatI).getGroupId());
                                globalchoiceWhatI++;
                            }
                            ...
                            if (sendsBuffer.get(0).getMembersSend().size() > lastview.getMembers().size()) {
                                ...
                                for (int j = 0; j < lastview.getMembers().size(); j++) {
                                    for (int i = 0; i < sendsBuffer.get(0).getMembersSend().size(); i++) {
                                        if (lastview.getMembers().get(j).getMemberPort() == sendsBuffer.get(0)
                                                .getMembersSend().get(i)) {
                                            resend = 1;
                                        }
                                    }
                            ...
                            BM_send(coordBuffer.get(globalchoicecoord),
                                    coordBuffer.get(globalchoicecoord).getGroupId());
                            globalchoicecoord++;
                            ...
                            if (resend.get(0).getMembersSend().size() > lastview.getMembers().size()) {
                                ...
                                for (int j = 0; j < lastview.getMembers().size(); j++) {
                                    for (int i = 0; i < resend.get(0).getMembersSend().size(); i++) {
                                        if (lastview.getMembers().get(j).getMemberPort() == resend.get(0)
                                                .getMembersSend().get(i)) {
                                            again = 1;
                                        }
                                    }
                    ...
                    while (sizeSends >= 0) {
                        ...
                        for (int i = 0; i < sendsBuffer.get(counter).getMembersSend().size(); i++) {
                            ...
                            for (int j = 0; j < view.getMembers().size(); j++) {
                                if (sendsBuffer.get(counter).getMembersSend().get(i) != view.getMembers().get(j)
                                        .getMemberPort()) {
                                    k++;
                                }
                            }
                            if (k == size) {
                                ...
                                if (sendsBuffer.get(counter).getMembersSend().size() == 0) {
                                    se++;
                                    // System.out.println("Took ack from everyone: " +
                                    // sendsBuffer.get(counter).getSeqNo() + " " +
                                    // sendsBuffer.get(counter).getMessage() + " " +
                                    // sendsBuffer.get(counter).getStartingSender());
                                    Message rec = new Message("", sendsBuffer.get(counter));
                                    receiveMiddle.add(rec);
                                    delete = 1;
                                    mids.get(sendsBuffer.get(counter).getStartingSender())
                                            .add(sendsBuffer.get(counter).getSeqNo());
                                }
                                if (delete == 1) {
                                    int p = -1;
                                    for (int j = 0; j < whatIsend.size(); j++) {
                                        if (whatIsend.get(j).getSeqNo() == sendsBuffer.get(counter).getSeqNo()
                                                && whatIsend.get(j).getStartingSender() == sendsBuffer.get(counter)
                                                        .getStartingSender()) {
                                            p = j;
                                        }
                                    }
                ...
                if (resend.get(counter).getGroupId() == view.getId()) {
                    ...
                    while (sizeresends >= 0) {
                        ...
                        for (int i = 0; i < resend.get(counter).getMembersSend().size(); i++) {
                            sizeresends--;
                            for (int j = 0; j < view.getMembers().size(); j++) {
                                if (resend.get(counter).getMembersSend().get(i) != view.getMembers().get(j)
                                        .getMemberPort()) {
                                    k++;
                                }
                            }
                            if (k == size) {
                                ...
                                if (resend.get(i).getMembersSend().size() == 0) {
                                    ...
                                    receiveMiddle.add(rec);
                                    delete = 1;
                                    rids.get(resend.get(counter).getStartingSender())
                                            .add(resend.get(counter).getSeqNo());
                                }
                            }
                            if (delete == 1) {
                                int p = -1;
                                for (int j = 0; j < whatIsendTotal.size(); j++) {
                                    if (whatIsendTotal.get(j).getSeqNo() == resend.get(counter).getSeqNo()
                                            && whatIsendTotal.get(j).getStartingSender() == resend.get(counter)
                                                    .getStartingSender()) {
                                        p = j;
                                    }
                                }
            ...
            try {
                ...
                for (int i = 0; i < msg.getMembersSend().size(); i++) {
                    if (msg.getMembersSend().get(i) == temp.getMemberPort()) {
                        ...
                        DatagramPacket packet2 = new DatagramPacket(byteMsg, byteMsg.length,
                                InetAddress.getByName(temp.getMemberAddress()), temp.getMemberPort());
                        counterUDP++;
    ...
    public void BM_deliverTotal1(UdpMessage receiveMessage) {
        ...
        if (receiveMessage.getTotaldelivered() < 0) {
            if (!mids.get(receiveMessage.getStartingSender()).contains(receiveMessage.getSeqNo())) {
                ...
                if (sendsBuffer.size() > 0) {
                    for (; i < sendsBuffer.size(); i++) {
                        if (sendsBuffer.get(i).getSeqNo() == receiveMessage.getSeqNo()
                                && sendsBuffer.get(i).getStartingSender() == receiveMessage.getStartingSender()) {
                            int l = -1;
                            ...
                            if (receiveMessage.getSenderPort() != OurPort) {
                                ...
                                int sender = -1;
                                for (int counter = 0; counter < receiveMessage.getMembersSend().size(); counter++) {
                                    if (receiveMessage.getMembersSend().get(counter) == receiveMessage
                                            .getSenderPort()) {
                                        sender = counter;
                                    }
                                }
                ...
                if (hold == 0) {
                    UdpMessage sendAddition = new UdpMessage(receiveMessage.getMessage(), receiveMessage.getSeqNo(),
                            receiveMessage.getSenderPort(), receiveMessage.getGroupId(),
                            receiveMessage.getStartingSender(), receiveMessage.getTotal(),
                            receiveMessage.getSelfDelivered(), -1);
                    //
                    for (int counter = 0; counter < receiveMessage.getMembersSend().size(); counter++) {
                    ...
                    // System.out.println(+size+ ""+ sendsBuffer.get(size -
                    // 1).getMembersSend().size());
                ...
                if (delete == 1) {
                    ...
                    for (int j = 0; j < whatIsendTotal.size(); j++) {
                        if (whatIsendTotal.get(j).getSeqNo() == receiveMessage.getSeqNo()
                                && whatIsendTotal.get(j).getStartingSender() == receiveMessage.getStartingSender()) {
                            p = j;
                        }
                    }
                    if (p >= 0) {
            ...
            if (!rids.get(receiveMessage.getStartingSender()).contains(receiveMessage.getSeqNo())) {
                ...
                for (; i < resend.size(); i++) {
                    if (resend.get(i).getSeqNo() == receiveMessage.getSeqNo()
                            && resend.get(i).getStartingSender() == receiveMessage.getStartingSender()) { // an to
                                                                                                          // sequenve
                                                                                                          // number
                                                                                                          // einai idio
                                                                                                          // kai to
                ...
                if (hold == 0) {
                    UdpMessage sendAddition = new UdpMessage(receiveMessage.getMessage(), receiveMessage.getSeqNo(),
                            receiveMessage.getSenderPort(), receiveMessage.getGroupId(),
                            receiveMessage.getStartingSender(), receiveMessage.getTotal(),
                            receiveMessage.getSelfDelivered(), receiveMessage.getTotaldelivered());
                ...
                if (delete == 1) {
                    ...
                    for (int j = 0; j < coordBuffer.size(); j++) {
                        if (coordBuffer.get(j).getSeqNo() == receiveMessage.getSeqNo()
                                && coordBuffer.get(j).getStartingSender() == receiveMessage.getStartingSender()) {
                            l = j;
                        }
                    }
                    if (l >= 0) {
        ...
        if (mids.get(receiveMessage.getStartingSender()).contains(receiveMessage.getSeqNo()) || delete == 1) {
            ...
            for (int counter = 0; counter < receiveMiddle.size(); counter++) {
                if (receiveMiddle.get(counter).getMessage().getSeqNo() == receiveMessage.getSeqNo()
                        && receiveMiddle.get(counter).getMessage().getSenderPort() == receiveMessage.getSenderPort()) {
                    k = counter;
                }
            }
            if (k >= 0) {
        ...
        if (!totalbufids.get(receiveMessage.getStartingSender()).contains(receiveMessage.getSeqNo())) {
            ...
            // +receiveMessage.getSeqNo()+"rece"+receiveMessage.getTotaldelivered());
            UdpMessage newAddi = new UdpMessage(receiveMessage.getMessage(), receiveMessage.getSeqNo(),
                    receiveMessage.getSenderPort(), receiveMessage.getGroupId(), receiveMessage.getStartingSender(),
                    receiveMessage.getTotal(), receiveMessage.getSelfDelivered(), receiveMessage.getTotaldelivered());
            for (int counter = 0; counter < receiveMessage.getMembersSend().size(); counter++) {
            ...
            for (int j = 0; j < middlewareTeamsBuffer.get(receiveMessage.getGroupId()).getMembers().size(); j++) {
                if (receiveMessage.getStartingSender() == middlewareTeamsBuffer.get(receiveMessage.getGroupId())
                        .getMembers().get(j).getMemberPort()) {
        ...
        if (totalbufids.get(receiveMessage.getStartingSender()).contains(receiveMessage.getSeqNo())) {
            if (receiveMessage.getTotaldelivered() >= 0) {
                for (int counter = 0; counter < totalbuf.size(); counter++) {
                    if (receiveMessage.getSeqNo() == totalbuf.get(counter).getMessage().getSeqNo() && receiveMessage
                            .getStartingSender() == totalbuf.get(counter).getMessage().getStartingSender()) {
        ...
        while (true) {
            ...
            for (int i = 0; i < totalbuf.size(); i++) {
                if (totalbuf.get(i).getMessage().getTotaldelivered() == totaldelivered) {
                    appids.get(totalbuf.get(i).getMessage().getStartingSender())
                            .add(totalbuf.get(i).getMessage().getSeqNo());
    ...
    public void BM_deliver(UdpMessage receiveMessage) {
        ...
        if (!mids.get(receiveMessage.getStartingSender()).contains(receiveMessage.getSeqNo())) {
            ...
            if (sendsBuffer.size() > 0) {
                for (; i < sendsBuffer.size(); i++) {
                    if (sendsBuffer.get(i).getSeqNo() == receiveMessage.getSeqNo()
                        ...
                        if (sendsBuffer.get(i).getMembersSend().size() == 0) {
                            se++;
                            // System.out.println("Took ack from everyone: mesa sthn bm_deliver"+
                            // receiveMessage.getSeqNo() +" "+ receiveMessage.getMessage() + " "
                            // +receiveMessage.getStartingSender());
                            Message rec = new Message("", receiveMessage);
                            receiveMiddle.add(rec);
                            delete = 1;
                            mids.get(receiveMessage.getStartingSender()).add(receiveMessage.getSeqNo());
            ...
            if (hold == 0) {
                UdpMessage sendAddition = new UdpMessage(receiveMessage.getMessage(), receiveMessage.getSeqNo(),
                        receiveMessage.getSenderPort(), receiveMessage.getGroupId(), receiveMessage.getStartingSender(),
                        receiveMessage.getTotal(), receiveMessage.getSelfDelivered(), -1);
                ...
                if (sendsBuffer.get(size - 1).getMembersSend().size() == 0) {
                    se++;
                    // System.out.println("Took ack from everyone: sthn prwth"+
                    // receiveMessage.getSeqNo() +" "+ receiveMessage.getMessage() + " "
                    // +receiveMessage.getStartingSender());
                    Message rec = new Message("", receiveMessage);
                    receiveMiddle.add(rec);
            ...
            if (delete == 1) {
                ...
                for (int j = 0; j < whatIsend.size(); j++) {
                    if (whatIsend.get(j).getSeqNo() == receiveMessage.getSeqNo()
                            && whatIsend.get(j).getStartingSender() == receiveMessage.getStartingSender()) {
                        p = j;
                    }
                }
                if (p >= 0) {
        ...
        if (!mbufids.get(receiveMessage.getStartingSender()).contains(receiveMessage.getSeqNo())) {
            mbufids.get(receiveMessage.getStartingSender()).add(receiveMessage.getSeqNo()); // prostikh ston buffer tou
                                                                                            // middleware gia auta poy
                                                                                            // perimenoume na pane sthn
                                                                                            // efarmogh
